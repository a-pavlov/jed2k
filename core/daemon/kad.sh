...
CLASS_HOME="$HOME/dev/server/jed2k/core/target"
...
CLASS=org.dkf.jed2k.Kad2
OPTIONS="--port=20001 --sport=20000 --workspace=/home/apavlov/dev/kad"
USER=apavlov
PID=/tmp/kad2.pid
LOG_OUT=$CLASS_HOME/kad2.log
LOG_ERR=$CLASS_HOME/kad2.err
PROCNAME=jed2k-0.0.1
LOGGING="-Djava.util.logging.config.file=/home/apavlov/dev/server/jed2k/core/src/main/resources/logging.properties"
 
#-outfile $LOG_OUT -errfile $LOG_ERR -pidfile

do_exec()
{
    $EXEC -nodetach -debug $LOGGING -home $JAVA_HOME -procname $PROCNAME -cp $CLASS_PATH  -pidfile $PID $1 $CLASS $OPTIONS
}
